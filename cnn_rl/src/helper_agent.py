...
class DQLAgent: # Here we introduce a class named as DQLAgent class, which encompasses all the methods used for training the DQL agent to make trading decisions
    ...
    #### TRAIN
    def train(self, env, episodes):  # here we take the env. object, of the class Portfolio env as an argument
        for episode in range(episodes):
            state, norm_state, images, labels = env.get_observation()  # we import the observations from the env, using the get_observation function
            env.reset()
            done = False
            total_reward = 0
            episode_returns = []
            ...
            while not done:
                weights, trade_signals = self.select_action(state, norm_state, images, labels)  # we take the action
                ...
                total_reward += port_return   # calc the reward
        ...
        return self.results  # return the results, which include, 
